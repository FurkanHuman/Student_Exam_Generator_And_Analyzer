...
public class CreateAnalysisCommand : IRequest<CreatedAnalysisResponse>, ISecuredRequest, ICacheRemoverRequest, ILoggableRequest, ITransactionalRequest
{
    ...
    public class CreateAnalysisCommandHandler : IRequestHandler<CreateAnalysisCommand, CreatedAnalysisResponse>
    {
        private readonly IMapper _mapper;
        private readonly IAnalysisRepository _analysisRepository;
        private readonly AnalysisBusinessRules _analysisBusinessRules;

        public CreateAnalysisCommandHandler(IMapper mapper, IAnalysisRepository analysisRepository,
        ...
        public async Task<CreatedAnalysisResponse> Handle(CreateAnalysisCommand request, CancellationToken cancellationToken)
        {
            Analysis analysis = _mapper.Map<Analysis>(request);

            await _analysisRepository.AddAsync(analysis);

            CreatedAnalysisResponse response = _mapper.Map<CreatedAnalysisResponse>(analysis);
            return response;
        }
    }
}