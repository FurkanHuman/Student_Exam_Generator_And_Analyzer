...
namespace Application.PdfPageProduct.AnalysisPageFeature.V1;
...
internal static class AnalsysPageV1MicroCalc
{
    ...
    internal static string ScoreColorForScore(QuestionScore questionScore) => ScoreColorForScore(questionScore.Score, questionScore.MaxScore);

    internal static IEnumerable<StudentQuizAnswer> SelectedStudents(Analysis analysisHeader, char[] crayzChar) => analysisHeader.StudentQuizAnswers.Where(s => !crayzChar.Any(c => s.SpecialStatuses.Contains(c)));

    internal static List<int> AverageScoresPieGraph(Analysis analysisHeader, char[] crayzChar) => ScoreSum(analysisHeader, crayzChar);
    ...
    internal static List<int> ScoreSum(Analysis analysisHeader, char[] crayzChar)
    {
        List<int> scores = new();
    ...
    internal static List<double> AverageScoresBarGraph(Analysis analysisHeader, char[] crayzChar)
    {
    ...
    internal static double CalculateClassAveragesPerQuestion(Analysis analysisHeader, int index, char[] crayzChar)
    {
        int totalScore = 0;
