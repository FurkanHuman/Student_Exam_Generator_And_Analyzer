...
namespace Application.PdfPageProduct.ExamPageFeature.V1;
    ...
    internal static void ExamHeader(Exam exam, PageDescriptor page)
    {
        page.Header().Height(3, Unit.Centimetre).Row(headerRow =>
        {
            ...
            headerRow.RelativeItem(5).Padding(0.2f, Unit.Centimetre).Column(generalInfo =>
            {
                ...
                generalInfo.Item().AlignCenter().Text($"{exam.Semester.BeginSemesterDate.Year}/{exam.Semester.EndSemesterDate.Year} - {exam.Semester.Name}");
                generalInfo.Item().AlignCenter().Text(exam.School.Name);
                generalInfo.Item().AlignCenter().Text(exam.LessonName);
                generalInfo.Item().AlignCenter().Text($"{exam.Semester.BeginSemesterDate} {exam.Semester.EndSemesterDate}").FontSize(12);
            });
