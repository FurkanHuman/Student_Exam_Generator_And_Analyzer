...
public class AnalysisAutomationServiceTest(IMediator mediatr, IExamService examService, IPrincipalService principalService, IAnalysisRepository analysisRepository)
{
    private const int DelayInDays = 0;
    ...
    {
        return eligibleExams.Items.GroupBy(e => (e.LessonId, e.Student.StudentClassId, e.ExamConfigurationId));
    }
    ...
    private async Task<IPaginate<Exam>?> GetExamsReadyForAnalysis(int activeSemesterId, DateOnly today, CancellationToken stoppingToken)
    {
        return await examService.GetListAsync(e => e.SemesterId == activeSemesterId
                                                && e.ExamDate.AddDays(DelayInDays) == today.AddDays(-1),
